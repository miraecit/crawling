...
(async () => {
    const brw = await puppeteer.launch({headless: false, args: ["--window-size=10,1080", "--window-position=480,0"]})
    const pge = await brw.newPage()
    await pge.setUserAgent('Mozilla/5.0 (iPhone; CPU iPhone OS 16_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.5 Mobile/15E148 Safari/604.1')

    let pagenation = 1 
    while (true) {
        const url = pagenation === 1 ? 
            'https://www.mining.com/commodity/lithium/' : 
            'https://www.mining.com/commodity/lithium/page/' + pagenation + '/#latest-section'
        ...
        for (const element of $(news).find('div > div > article > div > h3')) {
            ...
            try {
                if (title !== "" && content !== "") {
                    ...
                    await connection.query(sqlStatement, ['mining', title.text(), content, 'lithum', url, created])
                }
            }
            catch (err) { 
