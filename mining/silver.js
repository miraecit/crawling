...
(async () => {
    const brw = await puppeteer.launch({headless: false, args: ["--window-size=0,1080", "--window-position=880,0"]})
    ...
    while (true) {
        const url = pagenation === 1 ? 
            'https://www.mining.com/commodity/silver/' : 
            'https://www.mining.com/commodity/silver/page/' + pagenation + '/#latest-section'
        ...
        for (const element of $(news).find('div > div > article > div > h3')) {
            ...
            try {
                if (title !== "" && content !== "") {
                    ...
                    await connection.query(sqlStatement, ['mining', title.text(), content, 'silver', url, created])
                }
            }
            catch (err) { 
