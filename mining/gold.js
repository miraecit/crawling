...
(async () => {
    const brw = await puppeteer.launch({headless: false, args: ["--window-size=10,1080", "--window-position=280,0"]})
    ...
    while (true) {
        const url = pagenation === 1 ? 
            ...
            'https://www.mining.com/commodity/gold/page/' + pagenation + '/#latest-section'
        ...
        for (const element of $(news).find('div > div > article > div > h3')) {
            ...
            try {
                if (title !== "" && content !== "") {
                    ...
                    await connection.query(sqlStatement, ['mining', title.text(), content, 'gold', url, created])
                }
            }
            catch (err) { 
